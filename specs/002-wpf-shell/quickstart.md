...
```powershell
# 發行 x64 可執行檔（Release）
dotnet publish src/Rivet.WPF/Rivet.WPF.csproj -c Release -r win-x64 -p:PublishSingleFile=false -o ./publish/wpf
```
輸出範例：`publish/wpf/Rivet.WPF.exe`
...
- 剪貼簿空白或為圖片時，顯示友善錯誤且程式不中斷。
- `dotnet build` 與 `dotnet publish` 成功產出可執行檔並可啟動流程。

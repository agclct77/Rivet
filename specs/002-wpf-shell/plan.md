...
建立 `Rivet.WPF` 手動啟動殼，沿用現有 Service/Infrastructure/DI（不再要求 Console 相容），提供主視窗按鈕觸發中↔英翻譯：讀剪貼簿→呼叫既有翻譯→顯示譯文並回寫剪貼簿。需保留錯誤提示與 Serilog 紀錄，並支援 `dotnet build/publish` 產生可執行檔。
...
**Testing**: 單元/整合測試使用 NUnit + NSubstitute；翻譯與剪貼簿管線沿用既有測試；WPF UI 自動化採 FlaUI(UIA3) + NUnit，測試類別以 STA/非平行執行  
...
**Constraints**: 無熱鍵/常駐/系統匣（本里程碑排除）；需維持剪貼簿回寫；必須沿用既有設定/日誌路徑；UI 單窗格，固定繁中提示；發行需支援 `-r win-x64 --self-contained true /p:PublishSingleFile=true`  
